...
router.post('/', requireAuth, async (req, res) => {
  ...
  try {
    ...
    const storeId        = cashier?.store_id || req.user.store_id;
    ...
    let saleStatus;
    if (payment_method === 'M-Pesa') saleStatus = 'pending_mpesa';
    else if (payment_method === 'Split' && mpesaPortionNum > 0) saleStatus = 'pending_split';
