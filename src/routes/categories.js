...
router.post('/brands', requireAuth, ADMIN, async (req, res) => {
  try {
    ...
    res.status(201).json({ id: r.insertId, message: 'Brand created' });
  } catch (err) {
    if (err.code === 'ER_DUP_ENTRY') return res.status(409).json({ error: 'Brand already exists for this type' });
...
router.delete('/brands/:id', requireAuth, ADMIN, async (req, res) => {
  try {
    ...
    await db.query('UPDATE brands SET is_active=0 WHERE id=?', [req.params.id]);
...
router.get('/subtypes', requireAuth, async (req, res) => {
  try {
    ...
    let sql = 'SELECT * FROM sub_types WHERE is_active=1';
    const vals = [];
...
router.post('/subtypes', requireAuth, ADMIN, async (req, res) => {
  try {
    ...
    res.status(201).json({ id: r.insertId, message: 'Sub-type created' });
  } catch (err) {
    if (err.code === 'ER_DUP_ENTRY') return res.status(409).json({ error: 'Sub-type already exists for this brand' });
...
router.delete('/subtypes/:id', requireAuth, ADMIN, async (req, res) => {
  try {
    ...
    await db.query('UPDATE sub_types SET is_active=0 WHERE id=?', [req.params.id]);
