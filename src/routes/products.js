...
router.post('/', requireAuth, ADMIN, async (req, res) => {
  try {
    ...
    // Determine store_id — always use active_store_id so product goes to the
    // currently selected store. super_admin in global mode (no active store) can
    // optionally pass store_id in the body, else product is unassigned (null).
    let storeId;
    if (req.user.active_store_id) {
...
router.post('/bulk-import', requireAuth, ADMIN, async (req, res) => {
  try {
    ...
    // Determine store_id — use active store, fallback to body store_id for global super_admin
    let storeId;
    if (req.user.active_store_id) {
      storeId = req.user.active_store_id;
    } else if (req.user.role === 'super_admin') {
...
router.post('/bulk-create', requireAuth, ADMIN, async (req, res) => {
  try {
    ...
    // Always use active_store_id as the authoritative store.
    // super_admin in global mode (no active store) falls back to item's store_id or null.
